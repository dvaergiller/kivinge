...
use ratatui::{
    layout::{Constraint, Layout}, style::{Style, Stylize}, symbols, widgets::{Block, Borders, List, ListDirection, Paragraph}, Frame
};
    ...
    Result<(), Error>
{
    let draw = |frame: &mut Frame| {
        ...
        let attachments: Vec<String> =
            content_details.parts.iter().map(|d| indent(2, &d.name)).collect();
